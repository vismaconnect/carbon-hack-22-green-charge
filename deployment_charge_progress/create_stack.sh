...
YAML_FILE=charge_progress.yaml
...
if $DEPLOY_LAMBDA
then
cp lambda_charge_progress.py lambda_function.py
...
aws cloudformation deploy --template-file $TMP_DIR/$YAML_FILE --stack-name ChargeProgressFunction$STAGE --parameter-overrides Stage=$STAGE Bucket=$BUCKET_FUNCTION BucketData=$BUCKET_DATA
fi
