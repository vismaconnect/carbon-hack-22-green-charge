...
if $DEPLOY_LAMBDA
then
cp lambda_calculate_avoided_emissions.py lambda_function.py
...
aws cloudformation deploy --template-file $TMP_DIR/$YAML_FILE --stack-name CalculateAvoidedEmissionsFunction$STAGE --parameter-overrides Stage=$STAGE Bucket=$BUCKET_FUNCTION BucketData=$BUCKET_DATA
fi
