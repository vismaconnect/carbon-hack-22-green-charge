...
YAML_FILE=calibrate_start.yaml
...
if $DEPLOY_LAMBDA
then
cp lambda_calibrate_start.py $TMP_DIR/lambda_function.py
...
aws cloudformation deploy --template-file $TMP_DIR/$YAML_FILE --stack-name CalibrateStartFunction$STAGE --parameter-overrides Stage=$STAGE Bucket=$BUCKET_FUNCTION
fi
