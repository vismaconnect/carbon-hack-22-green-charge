...
if $DEPLOY_LAMBDA
then
cp lambda_charge_device.py lambda_function.py
...
aws cloudformation deploy --template-file $TMP_DIR/$YAML_FILE --stack-name ChargeDeviceFunction$STAGE --parameter-overrides Stage=$STAGE Bucket=$BUCKET_FUNCTION BucketData=$BUCKET_DATA
fi
