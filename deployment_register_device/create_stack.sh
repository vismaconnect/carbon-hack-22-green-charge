...
YAML_FILE=register_device.yaml
...
if $DEPLOY_LAMBDA
then
cp lambda_register_device.py lambda_function.py
...
aws cloudformation deploy --template-file $TMP_DIR/$YAML_FILE --stack-name RegisterDeviceFunction$STAGE --parameter-overrides Stage=$STAGE Bucket=$BUCKET_FUNCTION BucketData=$BUCKET_DATA
fi
